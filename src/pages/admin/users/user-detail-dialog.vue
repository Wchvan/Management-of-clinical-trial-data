    ...
    <el-dialog v-model="centerDialogVisible" width="40%" center destroy-on-close>
        <el-card class="box-card">
            ...
            <div v-for="(item, key, index) in userData" :key="index" class="text-lg py-2 flex flex-row">
                ...
                <span v-if="!inputFlag[index]" @click="inputFlag[index] = true">{{ item }}</span>
                <el-input v-if="key !== 'role' && inputFlag[index]" @blur="inputFlag[index] = false" v-model="userData[key]"
                    @change="changeFlag = true" style="font-size: 1.125rem;" ></el-input>
                <el-select v-if="key === 'role' && inputFlag[index]" v-model="userData[key]" placeholder="Select"
                    @blur="inputFlag[index] = false" @change="changeFlag = true" >
...
/* 修改相关 */
const inputFlag = ref<boolean[]>(new Array(Object.keys(dataIndexMap.value).length).fill(false)) // 用于判断是span还是input框
